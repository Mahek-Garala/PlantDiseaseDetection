...
const HomePage = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      // 1. Upload to Flask API for prediction
      ...
      const detectedDisease = response.data.disease;
      setDisease(detectedDisease);

      // 2. Now save image and disease info to backend (database + wwwroot folder)
      const saveFormData = new FormData();
      saveFormData.append("image", image);
      saveFormData.append("diseaseName", detectedDisease);
      const token = localStorage.getItem("token");
      await axios.post("http://localhost:5015/api/plant/save", saveFormData, {
        headers: {
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <header className="text-center py-12 bg-green-100">
        ...
        <p className="mt-2 text-gray-600">
          Upload an image and get instant disease diagnosis
        </p>
      </header>
      <main className="flex-1 flex flex-col items-center py-10 px-4">
        ...
        {preview && (
          <img
            src={preview}
            alt="Preview"
            className="mt-4 w-64 h-64 object-cover border"
          />
        )}
        <button
